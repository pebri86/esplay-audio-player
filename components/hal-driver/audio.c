 ...
 *
 * Scales the audio samples by the given volume factor and clips to prevent
 * overflow.
 *
...
/**
 * @brief Terminate the audio subsystem.
 *
 * Disables and deletes the I2S channel, cleaning up resources.
 *
 * @return 0 on success, -1 on failure.
 */
int audio_terminate()
{
    if (initialized) {
        ...
        if (i2s_del_channel(tx_chan) != ESP_OK)
            return -1;
 ...
 */
int audio_volume_get() { return audio_volume; }
 ...
 *
 * Validates parameters and logs reconfiguration requests, but does not actually
 * reconfigure as it's not supported in this ESP-IDF version.
 *
...
int audio_set_sample_rate(uint32_t rate, uint32_t bits, i2s_slot_mode_t ch) {
  ...
  if (rate == 0 || (bits != 16 && bits != 24 && bits != 32)) {
    ESP_LOGE(TAG, "Invalid sample rate or bit width: rate=%lu, bits=%lu",
             (unsigned long)rate, (unsigned long)bits);
    return -1;
  }
  ...
  // Reconfiguration is not supported in this ESP-IDF version, log and return
  // success to avoid errors
  ESP_LOGW(TAG,
           "Sample rate reconfiguration requested: rate=%lu, bits=%lu, ch=%d, "
           "but not supported. Keeping current config.",
           (unsigned long)rate, (unsigned long)bits, (int)ch);
  return 0;
}
