 ...
 *
 * Defines the application context structure and functions for initialization and deinitialization.
 */
 ...
 *
 * Holds all the global state for the application, including LVGL, playlist, UI, and keypad data.
 */
typedef struct {
    // LVGL
    ...
    // Playlist
    playlist_manager_t playlist; /**< Playlist manager instance. */

    // UI
    lv_obj_t *current_playing; /**< LVGL object for current playing display. */

    // Audio Player
    int skip_direction; /**< Direction for skipping tracks (-1 prev, 1 next, 0 none). */
