 ...
 *
 * Implements the LVGL-based user interface for the audio player, including
 * controls and metadata display.
 */
...
void ui_player_set_metadata(const char *title, const char *artist,
  ...
  lv_label_set_text(label_title, (title && *title) ? title : "Unknown Title");
 ...
 *
 * Builds the LVGL UI with header, cover, song info, controls, and volume
 * slider.
 *
...
void ui_player_create(lv_obj_t *parent, ui_player_btn_cb_t cb,
  ...
  /* ================= CONTROLS ================= */
  const char *btns[] = {LV_SYMBOL_PREV, LV_SYMBOL_PAUSE, LV_SYMBOL_NEXT, NULL};
  ...
  lv_obj_align(vol_value, LV_ALIGN_BOTTOM_RIGHT, UI_VOLUME_VALUE_X,
               UI_VOLUME_VALUE_Y);
  ...
  lv_obj_align(vol_icon, LV_ALIGN_BOTTOM_RIGHT, UI_VOLUME_ICON_X,
               UI_VOLUME_ICON_Y);
  ...
  lv_obj_align(vol, LV_ALIGN_BOTTOM_RIGHT, UI_VOLUME_SLIDER_X,
               UI_VOLUME_SLIDER_Y);
